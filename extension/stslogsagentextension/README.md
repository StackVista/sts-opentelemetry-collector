...
`stslogsagent` owns the fixed Promtail route, health probes, effective configuration
...
pipelines and the single route observer are initialized. Shutdown marks the agent
unready and fixes one absolute drain deadline. The connector joins admitted calls;
...
The graph requires one Filelog input and one Promtail export pipeline, joined by
`stslogsroute` with `controller_extension` pointing here. Enable
