...
func TestComponentFactoryType(t *testing.T) {
	require.Equal(t, "stsservicegraph", NewFactory().Type().String())
}
...
func TestComponentLifecycle(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name+"-shutdown", func(t *testing.T) {
			c, err := test.createFn(context.Background(), connectortest.NewNopSettings(metadata.Type), cfg)
			require.NoError(t, err)
			err = c.Shutdown(context.Background())
			require.NoError(t, err)
		})
		t.Run(test.name+"-lifecycle", func(t *testing.T) {
			firstConnector, err := test.createFn(context.Background(), connectortest.NewNopSettings(metadata.Type), cfg)
			require.NoError(t, err)
			...
			secondConnector, err := test.createFn(context.Background(), connectortest.NewNopSettings(metadata.Type), cfg)
			require.NoError(t, err)
