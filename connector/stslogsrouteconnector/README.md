...
`logsagent.Controller` extension at startup. The fixed agent selects the Promtail
pipeline. The connector does not retry exports.
...
exporter shutdown. The per-call bound is the exporter's retry budget plus attempt timeout, with 20 seconds of overhead in the lifetime.
