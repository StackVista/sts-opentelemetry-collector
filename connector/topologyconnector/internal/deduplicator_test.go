...
func TestProjectionHash_DeterministicOverAttributeOrder(t *testing.T) {
	...
	h1 := d.hasher.ProjectionHash("m1", settingsproto.METRICS, ctx1, ref)
	h2 := d.hasher.ProjectionHash("m1", settingsproto.METRICS, ctx2, ref)
...
func TestProjectionHash_MappingIdentifierIsolation(t *testing.T) {
	refSummary1 := newExpressionRefSummaryForSignal(
		settingsproto.TRACES, "mapping-A", types.ExpressionRefSummary{})
	refSummary2 := newExpressionRefSummaryForSignal(
		settingsproto.TRACES, "mapping-B", types.ExpressionRefSummary{})
	...
	h1 := d.hasher.ProjectionHash("mapping-A", settingsproto.TRACES, ctx, nil)
	h2 := d.hasher.ProjectionHash("mapping-B", settingsproto.TRACES, ctx, nil)
...
func TestProjectionHash_NilVsMissing(t *testing.T) {
	...
	h1 := d.hasher.ProjectionHash("m1", settingsproto.METRICS, ctx1, ref)
	h2 := d.hasher.ProjectionHash("m1", settingsproto.METRICS, ctx2, nil)
...
func TestShouldSend_KeyChangesWhenReferencedInputChanges(t *testing.T) {
	...
	send1 := d.ShouldSend("m1", settingsproto.METRICS, ctx, time.Minute)
	require.True(t, send1)

	// Second call with same inputs -> should not Send (refresh window not reached)
	send2 := d.ShouldSend("m1", settingsproto.METRICS, ctx, time.Minute)
	...
	send3 := d.ShouldSend("m1", settingsproto.METRICS, ctx, time.Minute)
...
func TestShouldSend_RefreshAfterThreshold(t *testing.T) {
	...
	refSummaries := newExpressionRefSummaryForSignal(settingsproto.TRACES, "m1", types.ExpressionRefSummary{})
	d := newDedup(t, 0.5, refSummaries)
	ctx := &ExpressionEvalContext{Resource: NewResource(map[string]any{"a": 1})}

	send1 := d.ShouldSend("m1", settingsproto.TRACES, ctx, ttl)
	require.True(t, send1)

	// Immediately should not Send again
	send2 := d.ShouldSend("m1", settingsproto.TRACES, ctx, ttl)
	require.False(t, send2)

	// Wait past refresh threshold (cacheTTL*refreshFraction)
	time.Sleep(ttl/2 + 20*time.Millisecond)

	send3 := d.ShouldSend("m1", settingsproto.TRACES, ctx, ttl)
	require.True(t, send3)
}
...
func newExpressionRefSummaryForSignal(
	signal settingsproto.OtelInputSignal, mappingIdentifier string, summary types.ExpressionRefSummary,
) map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary {
	return map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary{
		signal: {
...
func mergeExpressionRefSummaries(
	summaries map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary,
	rest ...map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary,
) map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary {
	result := make(map[settingsproto.OtelInputSignal]map[string]*types.ExpressionRefSummary)
