...
# Use a temp file instead of "sed -i" so it works on both GNU (Linux) and BSD (macOS) sed
sed 's|import "topo_stream/v1/|import "|g' spec/topo_stream_metadata.proto > spec/topo_stream_metadata.proto.tmp
mv spec/topo_stream_metadata.proto.tmp spec/topo_stream_metadata.proto
rm -rf "$CHECKOUT_DIR"
