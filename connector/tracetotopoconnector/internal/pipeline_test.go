...
func TestPipeline_ConvertSpanToTopologyStreamMessage(t *testing.T) {
	...
	t.Run("Convert traces to Components and Relation", func(t *testing.T) {
		...
		result := ConvertSpanToTopologyStreamMessage(traces, componentMappings, relationMappings, now)
		assert.Equal(t, expected, result)
	})

	t.Run("Reject components with unmatched conditions", func(t *testing.T) {
		componentMappings := []settings.OtelComponentMapping{
			{
				Conditions: &[]settings.OtelConditionMapping{
	...
	t.Run("Returns error when an attribute is missing", func(t *testing.T) {
		componentMappings := []settings.OtelComponentMapping{
			{
				Conditions: &[]settings.OtelConditionMapping{
					{Action: settings.CREATE, Expression: settings.OtelBooleanExpression{Expression: "vars.instanceId"}},
