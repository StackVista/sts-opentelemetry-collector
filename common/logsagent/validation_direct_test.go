...
func directFixtureMap(t *testing.T, fixture string) map[string]any {
	t.Helper()
...
func directFixtureSet(t *testing.T, values map[string]any, path string, value any) {
	t.Helper()
...
func directFixtureDelete(t *testing.T, values map[string]any, path string) {
	t.Helper()
	keys := strings.Split(path, "::")
	for _, key := range keys[:len(keys)-1] {
		var ok bool
...
func TestDirectValidatePipelineConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "spec graph", want: base},
		{
			...
			fixture: strings.NewReplacer("filelog/", "file_log/", "k8sattributes", "k8s_attributes").Replace(directPipelineFixture),
			want:    base,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fixture := tt.fixture
			if fixture == "" {
				fixture = directPipelineFixture
			}
			values := directFixtureMap(t, fixture)
			for path, value := range tt.changes {
				directFixtureSet(t, values, path, value)
			}
...
func TestDirectValidatePipelineConfigRejectsExporterChanges(t *testing.T) {
	tests := []struct {
		path  string
		value any
		want  string
	...
	for _, exporter := range []string{"stsk8slogs/promtail"} {
		for _, tt := range tests {
			t.Run(exporter+"/"+tt.path+"/"+tt.want, func(t *testing.T) {
				values := directFixtureMap(t, directPipelineFixture)
				directFixtureSet(t, values, "exporters::"+exporter+"::"+tt.path, tt.value)
				directAssertPipelineRejected(t, values, tt.want)
			})
		}
		for _, field := range []string{
			...
			t.Run(exporter+"/missing/"+field, func(t *testing.T) {
				values := directFixtureMap(t, directPipelineFixture)
				directFixtureDelete(t, values, "exporters::"+exporter+"::"+field)
				directAssertPipelineRejected(t, values, "")
			})
		}
	}
}

func TestDirectValidatePipelineConfigRejectsGraphChanges(t *testing.T) {
	tests := []struct {
		name  string
		path  string
		value any
		...
		{"connector graph", "connectors", map[string]any{"stslogsroute/logs": nil}, "route"},
		{"malformed connectors", "connectors", "invalid", "connectors"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			values := directFixtureMap(t, directPipelineFixture)
			directFixtureSet(t, values, tt.path, tt.value)
			directAssertPipelineRejected(t, values, tt.want)
		})
	}
	for _, path := range []string{
		...
		t.Run("missing/"+path, func(t *testing.T) {
			values := directFixtureMap(t, directPipelineFixture)
			directFixtureDelete(t, values, path)
			directAssertPipelineRejected(t, values, "")
		})
	}
}

func TestDirectValidatePipelineConfigDurationScalars(t *testing.T) {
	for _, path := range []string{
		"exporters::stsk8slogs/promtail::delivery::export_lifetime",
		...
		t.Run(path, func(t *testing.T) {
			for _, value := range []any{
				...
				values := directFixtureMap(t, directPipelineFixture)
				directFixtureSet(t, values, path, value)
				directAssertPipelineRejected(t, values, "positive duration")
			}
		})
	}
}

func TestDirectValidatePipelineConfigOverflow(t *testing.T) {
	tests := []struct {
		name    string
		timeout string
		elapsed string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			values := directFixtureMap(t, directPipelineFixture)
			directFixtureSet(t, values, "exporters::stsk8slogs/promtail::delivery::export_lifetime", "9223372036854775807ns")
			directFixtureSet(t, values, "exporters::stsk8slogs/promtail::timeout", tt.timeout)
			directFixtureSet(t, values, "exporters::stsk8slogs/promtail::retry_on_failure::max_elapsed_time", tt.elapsed)
			directAssertPipelineRejected(t, values, tt.want)
		})
	}
}

func TestDirectValidatePipelineConfigErrorsDoNotEchoValues(t *testing.T) {
	...
	for _, path := range []string{
		...
		values := directFixtureMap(t, directPipelineFixture)
		directFixtureSet(t, values, path, marker)
		message := directAssertPipelineRejected(t, values, "")
...
func TestDirectValidatePipelineConfigEmpty(t *testing.T) {
...
func TestDirectEffectiveConfigOmittedRecreate(t *testing.T) {
	values := directFixtureMap(t, directPipelineFixture)
	directFixtureDelete(t, values, "extensions::file_storage/logs::recreate")
	directFixtureSet(t, values, "exporters::stsk8slogs/promtail::sending_queue", nil)
	directFixtureSet(t, values, "exporters::stsk8slogs/promtail::sending_queue", nil)
	conf := confmap.NewFromStringMap(values)
	before := directSnapshotConfig(t, conf)
	...
	if before != directSnapshotConfig(t, conf) {
...
func TestDirectEffectiveConfigRejectsUnsafeValues(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.path, func(t *testing.T) {
			values := directFixtureMap(t, directPipelineFixture)
			directFixtureSet(t, values, tc.path, tc.value)
...
func directAssertPipelineRejected(t *testing.T, values map[string]any, want string) string {
	t.Helper()
	conf := confmap.NewFromStringMap(values)
	before := directSnapshotConfig(t, conf)
...
func directSnapshotConfig(t *testing.T, conf *confmap.Conf) string {
	t.Helper()
